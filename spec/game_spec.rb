...
describe Game do
  ...
  describe '#add_nodes_to_tree' do
    let(:root_node) { double(TreeNode, position: [0, 0]) }
    let(:nodes_queue) { game.instance_variable_get(:@nodes_queue) }
    before do
      game.instance_variable_set(:@nodes_queue, [root_node])
    end

    context 'when @finish_position is a first child of root' do
      let(:child_node) { double(TreeNode, position: [1, 2]) }
      before do
        allow(nodes_queue).to receive(:shift).and_return(root_node)
        allow(game).to receive(:add_children_to_node) { nodes_queue << child_node }
        game.instance_variable_set(:@finish_position, [1, 2])
      end

      it 'stops on the first iteration' do
